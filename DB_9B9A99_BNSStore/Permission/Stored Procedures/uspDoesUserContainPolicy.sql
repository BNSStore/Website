    ...
    BEGIN
	   ...
	   IF @UserID IS NULL
		  ...
		  IF @PolicyID IS NULL
			 ...
			 IF(SELECT TOP 1 [UserID] FROM [Permission].[UserPolicy]
			    WHERE [UserID] = @UserID AND
					[PolicyID] = @PolicyID) IS NOT NULL
				BEGIN
				    SET @Contains = 1;
				END;
			 ELSE
				BEGIN
				    ...
				    IF(SELECT TOP 1 [rp].[RoleID] FROM [Permission].[RolePolicy] [rp]
					  WHERE [rp].[PolicyID] = @PolicyID AND
						   [rp].[RoleID] IN(
										SELECT [ur].[RoleID] FROM [Permission].[UserRole] [ur]
										WHERE [ur].[UserID] = @UserID)) IS NOT NULL
					   BEGIN
						  SET @Contains = 1;
					   END;
				    ELSE
							...
							@PolicyName != '*'
							 BEGIN
								...
								IF @PolicyName IS NULL
								    ...
								    SET @Index = LEN(@PolicyName) -
															 (CHARINDEX('.', REVERSE(@PolicyName)) - 1
															 );
								    ...
								    ELSE
									   ...
									   EXEC [Permission].[uspDoesUserContainPolicy]
										   @UserID = @UserID,
										   @PolicyName = @Part,
